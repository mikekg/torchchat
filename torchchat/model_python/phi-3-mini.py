...
from transformers import AutoModelForCausalLM, DynamicCache # , AutoTokenizer, pipeline
...
class ModelWrapper(nn.Module):
    ...
    def forward(self, x: torch.Tensor, input_pos: Optional[torch.Tensor] = None) -> torch.Tensor:
        with torch.no_grad():
            ...
            outputs = self.model.forward(input_ids=x, position_ids=input_pos.unsqueeze(0), past_key_values=self.cache, use_cache=True, attention_mask=attention_mask, return_dict=True)
